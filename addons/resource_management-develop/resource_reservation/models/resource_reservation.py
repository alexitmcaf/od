...
class ResourceReservation(models.Model):
    ...
    def name_get(self):
        result = []
        for record in self:
            display_name = f"{record.title} - {record.name}"
    ...
    def update_booking_status_cancel(self):
        for reservation in self:
            # Check if the current user is the creator or the resource owner
            if reservation.create_uid.id == self.env.user.id or reservation.resource_name.resource_owner.id == self.env.user.id:
                reservation.write({'booking_status': 'cancelled'})
                reservation.send_confirmation_email()
            else:
                raise exceptions.ValidationError(_("You do not have permission to cancel this reservation."))
    ...
    def write(self, vals):
        ...
        if any(rec.booking_status in ['confirmed', 'cancelled'] for rec in self):
            raise exceptions.ValidationError(
                _("You cannot modify a reservation that is in a Confirmed or Cancelled."))
